...
public class Deck {
  ...
  private static ArrayList<Integer> listy =
      new ArrayList<>(); // this two arraylist is to store the randomize number
                         // and the card from the deck
  private static ArrayList<String> cunny =
      new ArrayList<>(); // which is mainly for testing only
  ...
  public void Largest_Card(
    ...
    String largest_card =
        holding.get(0); // to the get the first card from the randomize list
    ...
    for (String cunnies : holding) {
      ...
      if (cardValue >
          largest_value) { // this check which card ranking is higher -- if
  ...
  public void addCardToHand(String card) { deck.remove(card); }

  public String toString() { return "Deck   : " + deck; }
